  ...
  describe 'POST #create' do
    ...
    context 'with minimum attributes' do
      ...
      it 'returns the new project' do
        ...
        expect(task['attributes']['startedAt']).to eq(Time.now.in_time_zone.as_json)
      end
    end
    ...
    context 'with planned date' do
      let(:payload) { {
        task: {
          label: 'My task',
          planned_at: Time.new(2017).in_time_zone.as_json,
        },
      } }
      ...
      it 'returns the new task' do
        ...
        expect(task['attributes']['startedAt']).to eq(Time.now.in_time_zone.as_json)
        expect(task['attributes']['plannedAt']).to eq(Time.new(2017).in_time_zone.as_json)
      end
    end
    ...
    context 'with finished date' do
      let(:payload) do
        {
          task: {
            label: 'My task',
            finished_at: Time.new(2017).in_time_zone.as_json,
          },
        }
      end
      ...
      it 'returns the new task' do
        ...
        expect(task['attributes']['startedAt']).to eq(Time.now.in_time_zone.as_json)
        expect(task['attributes']['plannedAt']).to eq(Time.now.in_time_zone.as_json)
        expect(task['attributes']['finishedAt']).to eq(Time.new(2017).in_time_zone.as_json)
      end
    end
