  ...
  describe '#state' do
    context 'when it is newed' do
      ...
      it 'does not accept started_at' do
        project.started_at = Time.new(2017, 1, 1)
        expect(project).to be_invalid
      end

      it 'does not accept paused_at' do
        project.paused_at = Time.new(2017, 1, 15)
        expect(project).to be_invalid
      end

      it 'does not accept due_at' do
        project.due_at = Time.new(2017, 2, 20)
        expect(project).to be_invalid
      end

      it 'does not accept finished_at' do
        project.finished_at = Time.new(2017, 1, 15)
        expect(project).to be_invalid
      end
    end
    ...
    context 'when it is started' do
      ...
      it 'requires due_at greater than started_at' do
        project.due_at = Time.new(2017, 1, 1)
        project.started_at = Time.new(2017, 1, 15)
        expect(project).to be_invalid
      end
    ...
    context 'when it is paused' do
      ...
      it 'requires paused_at greater than started_at' do
        project.paused_at = Time.new(2017, 1, 1)
        project.started_at = Time.new(2017, 1, 15)
        expect(project).to be_invalid
      end
    ...
    context 'when it is finished' do
      ...
      it 'requires finished_at greater than started_at' do
        project.finished_at = Time.new(2017, 1, 1)
        project.started_at = Time.new(2017, 1, 15)
        expect(project).to be_invalid
      end
  ...
  describe '#update_with_transition!' do
    ...
    context 'when starting a newed project' do
      ...
      it 'updates due_at attribute' do
        expect(subject.reload.due_at).to eq(Time.new(2017, 2, 20))
      end
    ...
    context 'when pausing a started project' do
      ...
      it 'sets paused_at attribute to today' do
        expect(subject.reload.paused_at).to eq(Time.new(2017, 1, 20))
      end
    end
    ...
    context 'when restarting a paused project' do
      let(:project) { create :project, :paused, started_at: Time.new(2017, 1, 1) }
      let(:params) { {
        state: 'started',
      } }
      ...
      it 'does not change started_at attribute' do
        expect(subject.reload.started_at).to eq(Time.new(2017, 1, 1))
      end
    end
    ...
    context 'when finishing a started project' do
      let(:project) { create :project, :started, started_at: Time.new(2017, 1, 1) }
      let(:params) { {
        state: 'finished',
        finished_at: Time.new(2017, 1, 15),
      } }
      ...
      it 'updates finished_at attribute' do
        expect(subject.reload.finished_at).to eq(Time.new(2017, 1, 15))
      end
